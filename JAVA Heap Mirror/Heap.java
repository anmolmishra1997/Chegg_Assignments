...
public class Heap {
    ...
    public void mirrorHeap(Heap h)
    {
		if (isEmpty()) {
		    System.out.println("Heap is Empty");
		    return;
		}
		h.printMirrorByLevel(0, 0);
    }

    private void printMirrorByLevel(int i, int level) {
		if (i > last) return;
		for (int j = 0; j < level; j++)
		    System.out.print("   ");
		System.out.println("Priority: " + h[i].key + " Content: "+h[i].content);
		printMirrorByLevel(2*i+2, level+1);
		printMirrorByLevel(2*i+1, level+1);
    }
    ...
    public void insert(Object o, int p){
